...
//забираем последний блок в html по тегу
//Иначе слишком много мусора в теле письма получается 
Функция ВытащитьПоТегу(ТекстВложения,Тег)
	
	НачалоТаблицы = СтрНайти(ТекстВложения,"<" + Тег,НаправлениеПоиска.СКонца);
	КонецТаблицы = СтрНайти(ТекстВложения,"</" + Тег,НаправлениеПоиска.СКонца);
	
	Результат = Сред(ТекстВложения,НачалоТаблицы,КонецТаблицы - НачалоТаблицы)+"</" + Тег+">";
	Возврат Результат;
...
Процедура У_ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель)
	...
	Для Каждого Вложение Из ПреобразоватьВСоответствие(Вложения, "Ключ", "Значение") Цикл
		...
		//ТекстыВложений = ТекстыВложений  + ВытащитьПоТегу(Чтение.Прочитать(),"TABLE");
		//ТекстыВложений = ВытащитьПоТегу(ТекстыВложений,"Table");
	КонецЦикла;
	...
	ПараметрыДоставки.ШаблонТекста = ТекстыВложений ; 
...
Процедура ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель = Неопределено)
	...
	Иначе
		У_ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель);
	КонецЕсли;	
		// Шаблоны тема и тела
